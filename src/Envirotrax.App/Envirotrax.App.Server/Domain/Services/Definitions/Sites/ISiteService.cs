...
public interface ISiteService : IService<Site, SiteDto>
{
    ...
    Task<IPagedData<SiteDto>> GetCsiComplianceAsync(PageInfo pageInfo, Query query, CancellationToken cancellationToken);
    Task UpdateCsiAssignmentAsync(int siteId, int? userId, CancellationToken cancellationToken);
}
