...
// Indexed by the Expired From select value. Index 0 ("All overdue") has no bucket because it applies no
// date predicate at all — matching V1's Case 0. The server gate (NeedsFogPermit && !OutOfArea) has no date
// clause either, so that option also returns sites not yet due; the per-row Days Overdue badge is what
...
export class FogPermitComplianceManagementComponent implements OnInit {
    ...
    public table: TableViewModel<Site> = {
        columns: [],
        query: { sort: { fogPermitExpirationDate: 'Asc' }, filter: [] }
    };
    ...
    constructor(
        ...
        this.downloadConfig = {
            ...
            endpoint: this._siteService.getFogPermitComplianceEndpoint(),
    ...
    private buildDownloadColumns(): DownloadColumn[] {
        ...
        columns.push({ field: 'fogPermitExpirationDate', caption: 'PermitDate' });
    ...
    public async getCompliance(): Promise<void> {
        try {
            this.table.isLoading = true;
            this.table.items = await this._siteService.getFogPermitCompliance(
    ...
    private getColumns(): TableColumn<Site>[] {
        ...
        const columns: TableColumn<Site>[] = [
            ...
            { field: 'fogPermitExpirationDate', caption: 'Permit Date', type: ColumnType.date, rowCssClass: 'align-top' },
    ...
    private buildQuery(): Query {
        ...
        return { sort: { fogPermitExpirationDate: this.sortBy === '0' ? 'Asc' : 'Desc' }, filter };
    }
    ...
    private buildDaysOverdueFilter(): QueryProperty[] {
        ...
        if (bucket.mode === 'greaterThan') {
            return [this.rangeFilter('fogPermitExpirationDate', [{ value: threshold, op: 'Lte' }])];
        }

        return [this.rangeFilter('fogPermitExpirationDate', [
    ...
    private sendAssignmentEmail(site: Site, userId: number): void {
        ...
        const subject = encodeURIComponent(`FOG Permit Account Assignment - ${site.businessName || site.accountNumber || ''}`);
