...
export class FogInspectorLicensesService {
    constructor(
    ...
    public async getLicenseTypes(query: Query = {}): Promise<InputOption<ProfessionalLicenseType>[]> {
        const url = this._urlResolver.resolveUrl('/api/fog/inspectors/licenses/types');
    ...
    public async getLicenses(id: number, pageInfo: PageInfo, query: Query): Promise<PagedData<ProfessionalUserLicense>> {
        const url = this._urlResolver.resolveUrl(`/api/fog/inspectors/${id}/licenses`);
