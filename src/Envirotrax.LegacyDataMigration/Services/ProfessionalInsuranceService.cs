...
public class ProfessionalInsuranceService
{
    private const string ScriptsFolder = "Scripts/ProfessionalInsurances";
    ...
    {
        _logger.LogInformation("--------- Starting migration of professional insurances ---------");

        await ExecuteSqlScriptsAsync();
        await MoveInsuranceFilesToAzureAsync();

        _logger.LogInformation("--------- Finished migration of professional insurances ---------");
    }
    ...
    private async Task ExecuteSqlScriptsAsync()
    {
        ...
        foreach (var file in scripts)
        {
            ...
            _logger.LogInformation("Imported professional insurances. Count: {count}", addedRows);
        }
